#!/usr/bin/env python3
"""Export Acadie.sol directory data into static website payloads.
...
Outputs in the sibling ``acadie_sol`` site repo:
...
def parse_hours_schedule(value: str) -> list[dict[str, str]]:
    ...
    day_pattern = r"Monday|Mon|Tuesday|Tues|Tue|Wednesday|Wed|Thursday|Thur|Thurs|Thu|Friday|Fri|Saturday|Sat|Sunday|Sun|Lundi|Lun|Mardi|Mar|Mercredi|Mer|Jeudi|Jeu|Vendredi|Ven|Samedi|Sam|Dimanche|Dim"
    day_re = re.compile(rf"(?i)\b({day_pattern})\b(?:\s*(?:-|–|—|to|à)\s*({day_pattern})\b)?")
    schedule: dict[int, str] = {}
    recognized = False
    for part in parts:
        matches = list(day_re.finditer(part))
        if not matches:
            if re.search(r"(?i)\b(open\s+)?every\s+day\b", part):
                segments = [(list(range(7)), re.sub(r"(?i)\b(open\s+)?every\s+day\b\s*[,:-]?\s*", "", part).strip())]
            else:
                continue
        else:
            segments = []
            for position, match in enumerate(matches):
                ...
                next_start = matches[position + 1].start() if position + 1 < len(matches) else len(part)
                remainder = part[match.end():next_start].strip(" ,:-")
                remainder = re.sub(r"(?i)^(?:open|hours?|from)\s*[:,-]?\s*", "", remainder).strip()
                segments.append((day_indexes, remainder))
        for day_indexes, remainder in segments:
            if not remainder:
                return []
...
def localized(value: str | dict | None, fallback: str = "") -> dict[str, str]:
    """Return the standard Acadie.sol language object: English, French, Shiac.

    Drafts and old records may still provide plain strings. We normalize those to
    ``en`` while keeping ``fr`` and ``shiac`` empty so editors can fill them later.
    """
    if isinstance(value, dict):
        return {key: clean_text(value.get(key, "")) for key in LANG_KEYS}
    return {"en": clean_text(value or fallback), "fr": "", "shiac": ""}
...
def build_item(
    *,
    ...
    return {
        ...
        "region_id": clean_text((meta or {}).get("region_id", "") or raw_location.get("region_id", "") or "acadie-bathurst"),
...
def parse_location(path: Path) -> dict:
    ...
    return {
        ...
        "region_id": clean_text(meta.get("region_id", "acadie-bathurst")),
        "street_id": clean_text(meta.get("street_id", "")),
        "address": clean_text(meta.get("address", "") or contact.get("address", "")),
        "municipality": clean_text(meta.get("municipality", "Bathurst")),
        "public_area": clean_text(meta.get("public_area", "Acadie-Bathurst")),
...
def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Export Acadie.sol public payloads into the static site repo.")
    parser.add_argument("--directory", type=Path, default=DEFAULT_DIRECTORY_ROOT, help="Directory repo root (default: this repo).")
    parser.add_argument("--site", type=Path, default=DEFAULT_SITE_ROOT, help="Website repo root to write into (default: sibling 'acadie_sol').")
